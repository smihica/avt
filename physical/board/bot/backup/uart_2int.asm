...
.UARTRX_ReadRx:
   ...
   tst  REG[UART_2_RX_BUFFER_REG], 0x00                    ; Read the data buffer to clear it.
...
IF(UART_2_BACKSPACE_ENABLE)                                ; Enable if backspace/delete mode
   cmp  A,UART_2_BACKSPACE_ENABLE                          ; Check for backspace character
   jnz  .UARTRX_IGNORE                                     ; If not, skip the backspace stuff
   cmp  [UART_2_bRxCnt],00h                                ; Check if buffer empty
   jz   .RESTORE_IDX_PP                                    ; 
   dec  [UART_2_bRxCnt]                                    ; Decrement buffer count by one.
   jmp  .RESTORE_IDX_PP
ENDIF                                                      ; 

.UARTRX_IGNORE:
IF(UART_2_RX_IGNORE_BELOW)                                 ; Ignore charaters below this value
   cmp  A,UART_2_RX_IGNORE_BELOW                           ; If ignore char is set to 0x00, do not
