...
export  PWM16_2_wReadPulseWidth
export _PWM16_2_wReadPulseWidth
export  PWM16_2_wReadCounter
export _PWM16_2_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wPWM16_2_ReadPulseWidth    ; deprecated
export _wPWM16_2_ReadPulseWidth    ; deprecated
export  wPWM16_2_ReadCounter       ; deprecated
export _wPWM16_2_ReadCounter       ; deprecated
...
_PWM16_2_WritePulseWidth:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[PWM16_2_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[PWM16_2_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PWM16_2_wReadPulseWidth
;
;  DESCRIPTION:
...
;    functions.
;
 PWM16_2_wReadPulseWidth:
_PWM16_2_wReadPulseWidth:
 wPWM16_2_ReadPulseWidth:                        ; this name deprecated
_wPWM16_2_ReadPulseWidth:                        ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[PWM16_2_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[PWM16_2_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PWM16_2_wReadCounter
;
;  DESCRIPTION:
...
;        functions.
;
 PWM16_2_wReadCounter:
_PWM16_2_wReadCounter:
 wPWM16_2_ReadCounter:                           ; this name deprecated
_wPWM16_2_ReadCounter:                           ; this name deprecated
   ...
   mov   A, reg[PWM16_2_COMPARE_MSB_REG]         ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[PWM16_2_COMPARE_LSB_REG]         ;
   push  A                                          ;  -stack frame now 2 bytes-
   PWM16_2_Stop_M                                ; Disable the PWM function
   M8C_SetBank1                                     ;
   mov   A, reg[PWM16_2_INPUT_LSB_REG]           ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 3 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[PWM16_2_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[PWM16_2_COUNTER_MSB_REG]         ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[PWM16_2_COMPARE_MSB_REG]         ; Stash the Count MSB on the stack
   push  A                                          ;  -stack frame is now 4 bytes
   mov   A, reg[PWM16_2_COUNTER_LSB_REG]         ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[PWM16_2_COMPARE_LSB_REG]         ; Stash the Count LSB on the stack
   ...
   mov   reg[PWM16_2_INPUT_LSB_REG], A           ;    and restore it
   M8C_SetBank0                                     ;
   PWM16_2_Start_M                               ; Now re-enable the PWM function
