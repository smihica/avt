...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_EnableInt
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_DisableInt
;
;  DESCRIPTION:
...
_UART_2_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask UART_2_TX_INT_REG, UART_2_TX_INT_MASK
   M8C_DisableIntMask UART_2_RX_INT_REG, UART_2_RX_INT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_SetTxIntMode(BYTE bTxIntMode)
;
;  DESCRIPTION:
...
;
 UART_2_SetTxIntMode:
_UART_2_SetTxIntMode:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_SetBank1
   and   A, UART_2_INT_MODE_TX_COMPLETE
   jz    .SetModeRegEmpty
   or    REG[UART_2_TX_FUNC_REG], bfFUNCTION_REG_TX_INT_MODE_BIT
   M8C_SetBank0
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret

.SetModeRegEmpty:
   and   REG[UART_2_TX_FUNC_REG], ~bfFUNCTION_REG_TX_INT_MODE_BIT
   M8C_SetBank0
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_UART_2_Start:
   ...
   mov   REG[UART_2_TX_CONTROL_REG], A
   mov   REG[UART_2_RX_CONTROL_REG], A
IF ( UART_2_RXBUF_ENABLE )
   call  _UART_2_CmdReset
ENDIF
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
_UART_2_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[UART_2_TX_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   and   REG[UART_2_RX_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
_UART_2_SendData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov REG[UART_2_TX_BUFFER_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bReadTxStatus
;
;  DESCRIPTION:
...
_UART_2_bReadTxStatus:
 bUART_2_ReadTxStatus:                           ; For backwards compatibility only
_bUART_2_ReadTxStatus:                           ; For backwards compatibility only
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[UART_2_TX_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bReadRxData
;
;  DESCRIPTION:
...
;
 UART_2_bReadRxData:
_UART_2_bReadRxData:
 bUART_2_ReadRxData:                             ; For backwards compatibility only
_bUART_2_ReadRxData:                             ; For backwards compatibility only
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[UART_2_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bReadRxStatus
;
;  DESCRIPTION:
...
_UART_2_bReadRxStatus:
 bUART_2_ReadRxStatus:                           ; For backwards compatibility only
_bUART_2_ReadRxStatus:                           ; For backwards compatibility only
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[UART_2_RX_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_TxIntMode
;
;  DESCRIPTION:
...
_UART_2_TxIntMode:
   ...
   and  A,UART_2_INT_MODE_TX_COMPLETE
   M8C_SetBank1
   cmp  A,UART_2_INT_MODE_TX_COMPLETE
   jz   .SetTxCmpMode

   and  reg[UART_2_TX_FUNC_REG],0xEF             ; Set Interrupt on Tx_Reg_Empty
   jmp  .TxIntMode_End

.SetTxCmpMode:                                       ; Set Interrupt on TX Complete
   or   reg[UART_2_TX_FUNC_REG],0x10
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_PutSHexByte
;
;  DESCRIPTION:
...
_UART_2_PutSHexByte:
   ...
   index UART_2_HEX_STR                ; Get Hex value
   call  UART_2_PutChar                ; Write data to screen
   pop   A                             ; Restore value
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_PutSHexInt
;
;  DESCRIPTION:
...
_UART_2_PutSHexInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   swap  A,X
   call  UART_2_PutSHexByte            ; Print MSB
   mov   A,X                           ; Move LSB into position
   call  UART_2_PutSHexByte            ; Print LSB
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.BufEmptyWaitLoop:
   tst REG[UART_2_TX_CONTROL_REG], UART_2_TX_BUFFER_EMPTY     ; Check Tx Status
   jz  .BufEmptyWaitLoop
   mov REG[UART_2_TX_BUFFER_REG], @Source        ; Write data to Tx Port
   endm
...
_UART_2_cGetChar:
   RAM_PROLOGUE RAM_USE_CLASS_1
   tst REG[UART_2_RX_CONTROL_REG],UART_2_RX_REG_FULL    ; Check if a character is ready
   jz  UART_2_cGetChar                              ; If not loop
   mov A, REG[UART_2_RX_BUFFER_REG]                 ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;-----------------------------------------------------------------------------
...
_UART_2_cReadChar:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov  A,REG[UART_2_RX_CONTROL_REG]                       ; Get Status of RX
   push A
   and  A,UART_2_RX_COMPLETE                               ; Check if a character is ready
...
.RX_DATA_RDY:
   ...
   and  A,(UART_2_RX_PARITY_ERROR | UART_2_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_iReadChar
;
...
_UART_2_iReadChar:
   ...
   and  A,(UART_2_RX_ERROR|UART_2_RX_REG_FULL)
   push A
   and  A,UART_2_RX_COMPLETE                               ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,UART_2_RX_NO_DATA                                ; Add no data flag
   swap A,X
   jmp  End_UART_2_iReadChar

.RX_GET_DATA:
   pop  A
   and  A,UART_2_RX_ERROR
   swap A,X
   mov  A,REG[UART_2_RX_BUFFER_REG ]                       ; Read data first, then
                                                           ; determine if data is valid

 End_UART_2_iReadChar:
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.PutStringLoop:
   ...
   call  UART_2_PutChar                          ; Send character to Tx port
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_CPutString
;
;  DESCRIPTION:
;     Send String out through UART TX port.
;-----------------------------------------------------------------------------
...
_UART_2_CPutString:
   ...
   romx                                          ; Get character from ROM
   jz    .End_CPutString
   call  UART_2_PutChar                          ; Print character
   pop   X
   pop   A
   inc   X                                       ; Inc LSB of pointer
   jnc   UART_2_CPutString                       ; Check for carry
   inc   A                                       ; Inc MSB of pointer
   jmp   UART_2_CPutString
...
_UART_2_CmdReset:
   ...
   RAM_SETPAGE_CUR >UART_2_aRxBuffer
   mov [UART_2_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >UART_2_bRxCnt
   mov [UART_2_bRxCnt], 0x00
   and [UART_2_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bCmdCheck
;
;  DESCRIPTION:
...
_UART_2_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >UART_2_fStatus
   mov A,  [UART_2_fStatus]
   and A, UART_2_RX_BUF_CMDTERM                  ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bErrCheck
;
;  DESCRIPTION:
...
_UART_2_bErrCheck:
   ...
   RAM_SETPAGE_CUR >UART_2_fStatus
   mov A,  [UART_2_fStatus]
   and A, UART_2_RX_BUF_ERROR                    ; Mask off Error status
   and [UART_2_fStatus], ~UART_2_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_bCmdLength
;
;  DESCRIPTION:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_szGetParam
;
;  DESCRIPTION:
;      Return next parameter from UART_2 Rx buffer
;-----------------------------------------------------------------------------
;
;  ARGUMENTS:  none
;
...
;     UART_2_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, UART_2_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See UART_2_bCmdCheck).
;
...
_UART_2_szGetParam:
   ...
   RAM_SETPAGE_CUR >ptrParam
   mov   A, <UART_2_aRxBuffer               ; Get address to receive buffer
...
.ParamLoop:                                 ; Now loop until end of parameter found.
   ...
   inc   [ptrParam]
   cmp   [ptrParam], (UART_2_RX_BUFFER_SIZE -1)   ; Check if we are at the end of buffer
   jz    .TerminateString
   mov   A, [X]                             ; Get next character
   jz    .End_GetNextParam
   cmp   A, UART_2_DELIMITER                ; Check if we have a delimiter
   ...
   mov   [ptrParam], (UART_2_RX_BUFFER_SIZE -1)   ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A, >UART_2_aRxBuffer                ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: UART_2_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 UART_2_szGetRestOfParams:
_UART_2_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
   mov  A, <UART_2_aRxBuffer                ; Get address to receive buffer
   add  A, [ptrParam]                       ; Add string offset
