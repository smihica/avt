...
Encoder::~Encoder()
{
    if (initialized) lame_close(lame);
}
...
int Encoder::encode(short* pcm_src, unsigned long pcm_frames, unsigned char* mp3_dest, unsigned long mp3_size)
{
    int ret = lame_encode_buffer(lame, pcm_src, pcm_src, (int)pcm_frames, mp3_dest, (int)mp3_size);
    return ret;
}
